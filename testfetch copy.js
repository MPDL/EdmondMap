          ...
          additionalInfos.forEach((info) => {
            ...
            if (info.label == "Geo-coordinates") {
              ...
              /**console.log("Coordinates: "+ coordinates)
                        ...
                        var latLng = coordinates.replace(" ", "").split(",")
                        var lat = parseFloat(latLng[0]);
                        var lng = parseFloat(latLng[1]);
                        var marker = L.marker([lat, lng]).addTo(map);

		                marker.bindPopup("<b>"+ coordinates +"</b>").openPopup();
                        */
            }
          });
          if (coords.length !== 0) {
...
function makeList(listContent) {
  ...
  listContent.forEach((element) => {
    ...
    listItem.innerHTML =
      ...
      "https://edmond.mpdl.mpg.de/imeji/collection/nd_EdbS5XwdDxT3D" +
      '" target="_blank">' +
      element.title +
      "</a>";
    listItem.onmouseover = function () {
      ...
      element.coordinates.forEach((value) => {
        ...
        marker
          .bindPopup(
            "<b>" + element.title + "</b><br>[" + value.coordinate + "]"
          )
          .openPopup();
      });
    };
